...
(function () {
    function ProposalViewCtrl($state, $scope, $rootScope, $log, sessionService, GetProposal, GetContract, DesignInfo, localStorageService, managedModal, ProposalEmail, EmailDocusign, DocusignStatus, DocusignUpdate, ProposalSign) {
            ...
            self.proposalId = $rootScope.proposalId;
                ...
                DesignInfo.get({pricingid: $rootScope.PricingQuoteId}).$promise
                    ...
                    .catch(function (error) {
                        ...
                        showErrorDialog('Get design api failed ');
                        self.loading = false;
                    ...
                    .catch(function () {
                        GetProposal.get({id: $rootScope.PricingQuoteId}).$promise
                            ...
                            .catch(function () {
                                ...
                                self.loading = false;
                                showErrorDialog('Proposal api failed');
                                $state.go('customerDetails');
                            });
                    });
                        ...
                        .catch(function () {
                            GetContract.get({id: $rootScope.PricingQuoteId}).$promise
                                ...
                                .catch(function () {
                                    ...
                                    self.loading = false;
                                    showErrorDialog('Contract api failed');
                                    $state.go('customerDetails');
                                });
                        });
                    }
        ...
        function validateEmail(){
         ...
         function EmailProposal(){
            ...
            ProposalEmail.save(emailpayload).$promise
                    .then(function () {
                        self.waitimplementer = false;
                        showErrorDialog("Proposal email successfully");
                    })
                    .catch(function (error) {
                        //self.loading = false;
        ...
        function SendContracttoHO(){
            ...
            EmailDocusign.save({PricingQuoteId: $rootScope.PricingQuoteId}).$promise
                        .then(function () {
                            ...
                            showErrorDialog('Contract email sent to homeowner');
                        })
                        .catch(function (error) {
                            //self.loading = false;
                            self.waitimplementer = false;
           ...
           if($rootScope.envelopeId){
                DocusignStatus.get({envelope: $rootScope.envelopeId}).$promise
                        ...
                        if(data.Status === 'completed'){
                            self.waitimplementer = false;
                            showErrorDialog("Contract has alredy been signed");  
                        }
                        else
                            DocusignOpenInLink();
                        });                
        ...
        function DocusignOpenInLink(){
            self.waitimplementer = true;
            EmailDocusign.save({PricingQuoteId: $rootScope.PricingQuoteId, EmbeddedSigning : "true", ProposalId :$rootScope.ProposalId}).$promise
                    .then(function (data) {
            ...
            if ($rootScope.envelopeId) {
                DocusignStatus.get({envelope: $rootScope.envelopeId}).$promise
                        .then(function (data) {
                            if (data.Status === 'completed') {
                                self.waitimplementer = false;
                                showErrorDialog("Proposal has alredy been signed");
                            } else
